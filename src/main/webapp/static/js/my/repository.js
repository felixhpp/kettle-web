...
function bindSelectOptopns(obj) {
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i=0; i<data.length; i++){
                obj.find("#repositoryType").append('<option value="' + data[i].repositoryTypeCode + '">' + data[i].repositoryTypeDes + '</option>');
            }
        },
        error: function () {
            alert("请求失败！请刷新页面重试");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i=0; i<data.length; i++){
                databaseAccessObj.append('<option value="' + data[i] + '">' + data[i] + '</option>');
            }
        },
        error: function () {
            alert("请求失败！请刷新页面重试");
...
function repositoryListRefresh() {
    $('#repositoryList').bootstrapTable('refresh', "database/getList.shtml");
}
